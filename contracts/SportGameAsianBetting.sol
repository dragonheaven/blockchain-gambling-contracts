...
contract SportGameAsianBetting is Ownable {
    ...
    event BetWithAsianDealer(address player, uint256 gameId, uint8 option, uint256 amount, int16[2] handicap, uint16[2] winRateHome, uint16[2] winRateAway);
    
    function SportGameAsianBetting(address _creditManager) public {
        creditManager = _creditManager;
    }
    
    function getInfo(uint256 _gameId) public constant returns (uint16[2] gameScore, uint8 result, uint8 status, uint256 betLimit, uint256 betTotalLimit, int16[2][] handicap, uint16[2][] winRateHome, uint16[2][] winRateAway, uint numOfAsianBetPlayers, uint256 asianBetTotal) {
        Game memory game = games[_gameId];
        return (game.gameScore, uint8(game.result), uint8(game.status), game.betLimit, game.betTotalLimit, game.handicap, game.winRateHome, game.winRateAway, game.asianBetPlayers.length, game.asianBetTotal);
    }
    
    function getPlayerAsianBet(uint256 _gameId, uint _index) public constant returns (address addr, uint8 betOption, uint256 betAmount, uint256 winAmount, int16[2] handicap, uint16[2] winRateHome, uint16[2] winRateAway) {
        require(_index < games[_gameId].asianBetPlayers.length);
        
        AsianBetPlayer memory player = games[_gameId].asianBetPlayers[_index];
        return (player.addr, uint8(player.betOption), player.betAmount, player.winAmount, player.handicap, player.winRateHome, player.winRateAway);
    }
    ...
    function start(uint256 _gameId, uint256 _betLimit, uint256 _betTotalLimit, int16[2][] _handicap, uint16[2][] _winRateHome, uint16[2][] _winRateAway) onlyOwner public returns (bool success) {
    ...
    function refundAll(uint256 _gameId) internal returns (bool success) {
        uint i = 0;
        CreditManager lbcm = CreditManager(creditManager);
    ...
    function performAsianBet(uint256 _gameId, uint16[2] _gameScore) internal returns (bool success) {
        ...
        for (i = 0; i < games[_gameId].asianBetPlayers.length; ++i) {
            handicapNatural = games[_gameId].asianBetPlayers[i].handicap[0] / MAGIC_NUMBER;
            handicapDecimal = games[_gameId].asianBetPlayers[i].handicap[0] % MAGIC_NUMBER;
            ...
            amount = 0;
            betAmount = games[_gameId].asianBetPlayers[i].betAmount;
            
            if (delta > 0 && games[_gameId].asianBetPlayers[i].betOption == AsianBetOption.HOME) { // home win -> bet home win
                amount = betAmount.add(betAmount.mul(games[_gameId].asianBetPlayers[i].winRateHome[0]).div(games[_gameId].asianBetPlayers[i].winRateHome[1]));
            } else if (delta < 0 && games[_gameId].asianBetPlayers[i].betOption == AsianBetOption.AWAY) { // away win -> bet away win
                amount = betAmount.add(betAmount.mul(games[_gameId].asianBetPlayers[i].winRateAway[0]).div(games[_gameId].asianBetPlayers[i].winRateAway[1]));
            } else if (delta == 0) { // draw
                if (handicapDecimal == 0) { // handicap 0 -> stake refund
                    amount = betAmount;
                } else if (handicapDecimal == 2 && games[_gameId].asianBetPlayers[i].betOption == AsianBetOption.HOME) { // handicap 0.5 -> bet home win
                    amount = betAmount.add(betAmount.mul(games[_gameId].asianBetPlayers[i].winRateHome[0]).div(games[_gameId].asianBetPlayers[i].winRateHome[1]));
                } else if (handicapDecimal == -2 && games[_gameId].asianBetPlayers[i].betOption == AsianBetOption.AWAY) { // handicap -0.5 -> bet away win
                    amount = betAmount.add(betAmount.mul(games[_gameId].asianBetPlayers[i].winRateAway[0]).div(games[_gameId].asianBetPlayers[i].winRateAway[1]));
                } else if (handicapDecimal == 1 || handicapDecimal == -3) { // handicap 0.25, -0.75 -> bet home half win, bet away half lose
                    if (games[_gameId].asianBetPlayers[i].betOption == AsianBetOption.HOME) {
                        amount = betAmount.add(betAmount.mul(games[_gameId].asianBetPlayers[i].winRateHome[0]).div(games[_gameId].asianBetPlayers[i].winRateHome[1] * 2));
                    } else if (games[_gameId].asianBetPlayers[i].betOption == AsianBetOption.AWAY) {
                    ...
                    if (games[_gameId].asianBetPlayers[i].betOption == AsianBetOption.HOME) {
                        ...
                        amount = betAmount.add(betAmount.mul(games[_gameId].asianBetPlayers[i].winRateAway[0]).div(games[_gameId].asianBetPlayers[i].winRateAway[1] * 2));
                    }
                }
            }
            
            if (amount > 0) {
                if (!lbcm.increaseCredit(games[_gameId].asianBetPlayers[i].addr, amount)) revert();
                games[_gameId].asianBetPlayers[i].winAmount = amount;
            }
        }
    ...
    function betWithAsianDealer(uint256 _gameId, uint8 _betOption, uint256 _betAmount, uint _handicapIndex) public returns (bool success) {
        ...
        games[_gameId].asianBetPlayers.push(AsianBetPlayer(msg.sender, AsianBetOption(_betOption), _betAmount, 0, games[_gameId].handicap[_handicapIndex], games[_gameId].winRateHome[_handicapIndex], games[_gameId].winRateAway[_handicapIndex]));
        games[_gameId].asianBetTotal = games[_gameId].asianBetTotal.add(_betAmount);
        
        BetWithAsianDealer(msg.sender, _gameId, _betOption, _betAmount, games[_gameId].handicap[_handicapIndex], games[_gameId].winRateHome[_handicapIndex], games[_gameId].winRateAway[_handicapIndex]);
